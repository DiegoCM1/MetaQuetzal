...
const DEV_BYPASS_MAP_AUTH =
  process.env.EXPO_PUBLIC_DEV_BYPASS_MAP_AUTH === "true";
...
function normalizeZone(event: MapEventResponse): Zone {
  return {
    ...
    timestamp: String(
      event.created_at ?? event.updated_at ?? new Date().toISOString(),
    ),
    radius: 500,
    type: event.type,
    userId: event.user_id ?? null,
    upvotes: Number(event.upvotes ?? 0),
    downvotes: Number(event.downvotes ?? 0),
    userVote: event.user_vote ?? null,
    isOwner: Boolean(event.is_owner),
  };
}
...
export async function createZone(zone: Zone) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      message || `Failed to create map event: ${response.status}`,
    );
  }
...
export async function updateZone(zone: Zone) {
  ...
  if (!response.ok) {
    const message = await response.text().catch(() => "");
    throw new Error(
      message || `Failed to update map event: ${response.status}`,
    );
  }
...
export async function deleteZone(zoneId: string) {
  ...
  if (!response.ok) {
    const message = await response.text().catch(() => "");
    throw new Error(
      message || `Failed to delete map event: ${response.status}`,
    );
  }
}
...
export async function voteZone(zoneId: string, value: 1 | -1) {
  ...
  if (!response.ok) {
    const message = await response.text().catch(() => "");
    throw new Error(
      message || `Failed to vote for map event: ${response.status}`,
    );
  }
}
